...
-- Password: test
INSERT INTO t_accounts (username, account_password, firstname, lastname, role_id) VALUES 
	('admin@localhost', '$2a$12$u92sdBoHe8dWBNTKdoKKZel6iG.p.keLnk3XoYMAbEt8w71ewDNFq', 'Admin', 'ADMIN', (SELECT r.id FROM t_roles r WHERE r.code = 'ROLE_ADMIN')),
	('trainer@localhost', '$2a$12$m/wGwkksyXt99kyWwU71o.Lc3N7GQQoM1vZFNDQv5bJqjtVnljHEu', 'Trainer', 'TRAINER', (SELECT r.id FROM t_roles r WHERE r.code = 'ROLE_TRAINER')),
	('skilltree-reset-test@readresolve.io', '$2a$12$lQR5FkxU5Bqveku0Iy.yNe1fBjZguQyotjP35iKLv5pN4OFHyJ4I2', 'Trainer', 'TRAINER', (SELECT r.id FROM t_roles r WHERE r.code = 'ROLE_TRAINER'));

INSERT INTO t_trainers (code, account_id) VALUES
	(concat('FO', to_char(nextval('trainers_code_seq'), 'FM00000000')), (SELECT a.id FROM t_accounts a WHERE a.username = 'trainer@localhost')),
	(concat('FO', to_char(nextval('trainers_code_seq'), 'FM00000000')), (SELECT a.id FROM t_accounts a WHERE a.username = 'skilltree-reset-test@readresolve.io'));
